...
is
   l_label t_128b;
begin
   l_label := utl_normalize_label(p_label);

   if g_counters.exists(l_label) then
      g_counters(l_label) := g_counters(l_label) + 1;
   else
      g_counters(l_label) := 1;
   end if;
end count;
...
begin
   ...
   if g_counters.exists(l_label) then
      if utl_logging_is_enabled (c_level_info) then
         l_log_id := utl_create_log_entry (
            p_level   => c_level_info,
            p_message => l_label || ': ' || to_char(g_counters(l_label)) ||
                           case
      ...
      warn('Counter `' || l_label || '` does not exist.');
   end if;
end count_current;
...
begin
   ...
   if g_timers.exists(l_label) then
      if utl_logging_is_enabled (c_level_info) then
         l_log_id := utl_create_log_entry (
            p_level   => c_level_info,
            p_message => l_label || ': ' || runtime (g_timers(l_label)) ||
                           case
                              when p_message is not null
                              then ' - ' || p_message
                           end );
      ...
      warn('Timer `' || l_label || '` does not exist.');
   end if;
end time_current;
...
begin
   ...
   if g_timers.exists(l_label) then
      l_return := runtime(g_timers(l_label));
   end if;
   return l_return;
end time_current;
...
function format (
   ...
   p9        in varchar2 default null )
return varchar2 is
   l_message t_32kb := p_message;
begin
   -- id replacements
   l_message := replace(l_message, '%0', p0);
   l_message := replace(l_message, '%1', p1);
   l_message := replace(l_message, '%2', p2);
   l_message := replace(l_message, '%3', p3);
   l_message := replace(l_message, '%4', p4);
   l_message := replace(l_message, '%5', p5);
   l_message := replace(l_message, '%6', p6);
   l_message := replace(l_message, '%7', p7);
   l_message := replace(l_message, '%8', p8);
   l_message := replace(l_message, '%9', p9);

   -- new line
   l_message := replace(l_message, '%n', c_lf);

   -- positional replacements
   return sys.utl_lms.format_message (
      l_message, p0, p1, p2, p3, p4, p5, p6, p7, p8, p9 );
end format;
...
procedure add_param (
   ...
   p_value in varchar2 )
is
   l_param t_attribute_value_row;
begin
   l_param.attribute := substr(p_name, 1, 128);
   l_param.value     := substr(p_value, 1, c_param_value_max_length);

   g_params(g_params.count + 1) := l_param;
end add_param;
...
begin
   l_param.attribute := substr(p_name, 1, 128);
   l_param.value     := to_char(p_value, 'yyyy-mm-dd hh24:mi:ss');
...
begin
   l_param.attribute := substr(p_name, 1, 128);
   l_param.value     := to_char(p_value, 'yyyy-mm-dd hh24:mi:ssxff');
...
begin
   l_param.attribute := substr(p_name, 1, 128);
   l_param.value     := to_char(p_value, 'yyyy-mm-dd hh24:mi:ssxff tzr');
...
begin
   l_param.attribute := substr(p_name, 1, 128);
   l_param.value     := substr(to_char(p_value), 1, c_param_value_max_length);
...
begin
   l_param.attribute := substr(p_name, 1, 128);
   l_param.value     := to_string(p_value);
...
is
begin
   return utl_logging_is_enabled(c_level_warning);
end;
...
is
begin
   return utl_logging_is_enabled(c_level_info);
end;
...
is
begin
   return utl_logging_is_enabled(c_level_debug);
end;
...
is
begin
   return utl_logging_is_enabled(c_level_trace);
end;
...
is
begin
   return to_yn(utl_logging_is_enabled(c_level_warning));
end;
...
is
begin
   return to_yn(utl_logging_is_enabled(c_level_info));
end;
...
is
begin
   return to_yn(utl_logging_is_enabled(c_level_debug));
end;
...
is
begin
   return to_yn(utl_logging_is_enabled(c_level_trace));
end;
...
is
   ...
   l_log_id          number;
   l_constraint_name t_256b;
   l_app_id          t_int := v('APP_ID');
   l_app_page_id     t_int := v('APP_PAGE_ID');
   --
         ...
         l_return :=
            ...
            replace (
               replace (
                  ...
                  rpad(l_app_id, 9, ' ') ),
               '#LOG_ID##########',
               rpad(nvl(to_char(l_log_id), 'this.log_id'), 17, ' ') ) ||
            case p_type
               when 'html' then '</pre>'
               when 'md'   then '```'
            end;
   ...
   is
      ...
      l_message_text t_1kb :=
         'DEVELOPER TODO: Change the message in APEX > Application Builder ' ||
         '> Shared Components > Text Messages for constraint ' ||
         p_constraint_name || '.';
   begin
   ...
   is
      ...
      l_cache t_32kb;
   begin
      clob_append ( l_clob, l_cache, p_text                   || c_lflf                                              );
      clob_append ( l_clob, l_cache, '## Technical Info'      || c_lflf                                              );
      clob_append ( l_clob, l_cache, '1. is_internal_error: ' || to_string(p_error.is_internal_error)        || c_lf );
      clob_append ( l_clob, l_cache, '2. apex_error_code: '   || p_error.apex_error_code                     || c_lf );
      clob_append ( l_clob, l_cache, '3. original message: '  || p_error.message                             || c_lf );
      clob_append ( l_clob, l_cache, '4. ora_sqlcode: '       || p_error.ora_sqlcode                         || c_lf );
      clob_append ( l_clob, l_cache, '5. ora_sqlerrm: '       || utl_replace_linebreaks(p_error.ora_sqlerrm) || c_lf );
      clob_append ( l_clob, l_cache, '6. component.type: '    || p_error.component.type                      || c_lf );
      clob_append ( l_clob, l_cache, '7. component.id: '      || p_error.component.id                        || c_lf );
      clob_append ( l_clob, l_cache, '8. component.name: '    || p_error.component.name                      || c_lf );
      clob_append ( l_clob, l_cache, '9. error_backtrace: '   || to_md_li_pre(p_error.error_backtrace)       || c_lf );
      clob_append ( l_clob, l_cache, '10. error_statement: '  || to_md_li_pre(p_error.error_statement)       || c_lf );
      clob_flush_cache ( l_clob, l_cache );
      return l_clob;
   end log_message;
   --
begin
   ...
   if p_error.is_internal_error then
      ...
      if not p_error.is_common_runtime_error then
         ...
         l_log_id := error (
            ...
            p_call_stack      => false                                                                            ,
            p_apex_env        => true                                                                             ,
            p_user_scope      => 'APEX BACKEND ERROR HANDLER: App ' || l_app_id || ', page ' || l_app_page_id     ,
         ...
         l_result.message :=
            case when l_result.display_location = apex_error.c_on_error_page then ascii_art('html') end ||
            ...
            '"App ID ' || to_char(l_app_id) || ', Log ID ' || to_char(l_log_id) ||
      ...
      l_result.display_location :=
         case when l_result.display_location = apex_error.c_on_error_page
      ...
      if p_error.ora_sqlcode in (-1, -2091, -2290, -2291, -2292) then
         l_constraint_name := extract_constraint_name(p_error.ora_sqlerrm);
         ...
         if l_result.message = l_constraint_name then
            ...
            l_result.message := create_apex_lang_message (l_constraint_name);

            -- Log a permanent error, so developers get information that they need
            -- to change the text message.
            error (
               p_message         => log_message (l_result.message)                                               ,
               p_permanent       => true                                                                         ,
               p_call_stack      => false                                                                        ,
               p_apex_env        => true                                                                         ,
               p_user_scope      => 'APEX BACKEND ERROR HANDLER: App ' || l_app_id || ', page ' || l_app_page_id ,
...
begin
   ...
   l_level           := to_number(apex_application.g_x01) ;
   l_message         := apex_application.g_x02            ;
   l_user_scope      := nvl(apex_application.g_x03, '.')  ;
   l_user_call_stack := nvl(apex_application.g_x04, '.')  ;
   l_user_agent      := apex_application.g_x05            ;
...
begin
   ...
   l_conf := utl_get_conf; -- this will handle the defaults if we don't have configured console yet.
   l_conf.conf_sysdate     := sysdate;
   l_conf.conf_user        := substrb(coalesce(sys_context('USERENV','OS_USER'), sys_context('USERENV','SESSION_USER')), 1, 64);
   l_conf.level_id         := coalesce(p_level, l_conf.level_id);
   l_conf.level_name       := level_name(l_conf.level_id);
   l_conf.check_interval   := coalesce(p_check_interval, l_conf.check_interval);
   l_conf.enable_ascii_art := to_string(coalesce(p_enable_ascii_art, to_bool(l_conf.enable_ascii_art)));

   assert (
      l_conf.level_id between c_level_error and c_level_trace,
      'Level needs to be 1 (error), 2 (warning), 3 (info), 4 (debug) or 5 (trace).');

   assertf (
      l_conf.check_interval between c_check_interval_default and c_check_interval_max,
...
begin
   ...
   l_prefs.client_identifier := p_client_identifier;
   l_prefs.level_id          := p_level;
   l_prefs.level_name        := level_name(p_level);
   l_prefs.call_stack        := to_string(p_call_stack);
   l_prefs.user_env          := to_string(p_user_env);
   l_prefs.apex_env          := to_string(p_apex_env);
   l_prefs.cgi_env           := to_string(p_cgi_env);
   l_prefs.console_env       := to_string(p_console_env);
   l_prefs.check_interval    := p_check_interval;
   l_prefs.exit_sysdate      := sysdate + 1/24/60 * p_duration;
...
is
   l_program      t_512b := substrb(replace(upper(p_program), ' ', '_'), 1, 512);
   l_object_name  t_128b := substrb(nvl(regexp_substr(l_program, '(.*+\.)?(.*)', 1, 1, 'i', 2), '-'), 1, 128);
   l_package_name t_128b := substrb(nvl(regexp_substr(l_program, '(.*)\.'      , 1, 1, 'i', 1), '-'), 1, 128);
   --
   c_return_value constant t_32b  := 'YOUR_RETURN_VALUE';
   l_object_is_a_function boolean := false;
   --
   cursor cur_args(
   ...
   l_args_in  t_args_tab;
   l_args_out t_args_tab;
  --
   procedure get_arguments
   is
      l_args t_args_tab;
   begin
      open cur_args(l_object_name, l_package_name);
      fetch cur_args bulk collect into l_args;
      for i in 1..l_ARGS.COUNT
      loop
         if l_args(i).in_out in ('IN', 'IN/OUT') then
            l_args_in(l_args_in.count + 1) := l_args(i);
         end if;

         if l_args(i).in_out in ('OUT', 'IN/OUT') then
            l_args_out(l_args_out.count + 1) := l_args(i);
         end if;

         if l_args(i).argument_name = c_return_value then
            l_object_is_a_function := true;
         end if;
      end loop;
   end get_arguments;
   --
   function params_proc_name (
      p_arg_type in varchar2 )
      return varchar2
   is
   begin
   ...
   --
   function params_proc_call (
      p_in_out in t_4b )
      return t_128b
   is
      l_return t_128b;
   begin
      if p_in_out = 'in'  and l_args_in.count  = 0
      or p_in_out = 'out' and l_args_out.count = 0 then
         l_return := format('--there are no %s parameters: ', upper(p_in_out));
      end if;

      l_return := l_return || params_proc_name(p_in_out);
   ...
   begin
      if p_args.count > 0 then
         printf('   procedure %s', params_proc_name(p_in_out));
         print ('   is');
         print ('   begin');
         print ('      console');
...
begin
   ...
   printf  ( '-- Your Program : %s'      , l_program                    );
   printf  ( '-- Package Name : %s'      , l_package_name               );
   printf  ( '-- Object Name  : %s'      , l_object_name                );
   print   ( '--------------------------------------------------------' );
   print   ( 'declare'                                                  );

   gen_params_proc('in' , l_args_in );
   gen_params_proc('out', l_args_out);
   ...
   if l_object_is_a_function then
      print ( '   ----------------------' );
   ...
   print   ( '      console.error;'                           );
   print   ( '      raise;'                                   );
   printf  ( 'end %s;'              , lower(l_object_name)    );
   print   ( '/'                                              );
...
is
   ...
   l_idx        t_int;
   l_sep_length t_int;
   l_return     t_vc2_tab_i;
begin
   if p_string is not null then
      if p_sep is null then
         for i in 1 .. length(p_string)
         loop
            l_return(l_return.count + 1) := substr(p_string, i, 1);
         end loop;
      else
         l_str        := p_string;
         ...
         loop
            l_idx := instr(l_str, p_sep);

            if l_idx > 0 then
               l_return(l_return.count + 1) := substr(l_str, 1, l_idx - 1);
               l_str := substr(l_str, l_idx + l_sep_length);
            else
               l_return(l_return.count + 1) := l_str;
               exit;
            end if;
         end loop;
      end if;
...
begin
   return
      case
         when upper(trim(p_string)) in ('TRUE', 'Y', 'YES', '1') then true
         when upper(trim(p_string)) in ('FALSE', 'N', 'NO', '0') then false
         else null
      end;
end to_bool;
...
function to_html_table (
   ...
   return clob
is
   l_data_cursor        sys_refcursor := p_data_cursor;
   l_cursor_id          integer;
   l_clob               clob;
   l_cache              t_32kb;
   l_data_count         t_int := 0;
   l_col_count          t_int;
   l_desc_tab           sys.dbms_sql.desc_tab3;
   l_buffer_varchar2    t_32kb;
   l_buffer_clob        clob;
   l_buffer_xmltype     xmltype;
   l_buffer_long_length t_int;
   --
   procedure close_cursor ( p_cursor_id in out integer )
   is
   begin
   ...
   begin
      ...
      loop
         if l_desc_tab(i).col_type = c_clob then
            sys.dbms_sql.define_column(l_cursor_id, i, l_buffer_clob);
         elsif l_desc_tab(i).col_type = c_xmltype then
            sys.dbms_sql.define_column(l_cursor_id, i, l_buffer_xmltype);
         elsif l_desc_tab(i).col_type = c_long then
            sys.dbms_sql.define_column_long(l_cursor_id, i);
   ...
   is
   begin
      clob_append(l_clob, l_cache, c_lf || '<tr><!--- header -->' || c_lf);

      if p_include_row_num then
         clob_append(l_clob, l_cache, '<th id="row_num">Row#</th>' || c_lf);
      end if;
      ...
      loop
         clob_append (
            l_clob,
            l_cache,
            '<th id="' || lower(l_desc_tab(i).col_name) || '">' ||
               initcap(replace(l_desc_tab(i).col_name, '_', ' ')) ||
               '</th>' || c_lf );
      end loop;

      clob_append(l_clob, l_cache, '</tr><!-- header -->' || c_lf);
   end create_header;
   --
   ...
   begin
      loop
         exit when sys.dbms_sql.fetch_rows(l_cursor_id) = 0 or l_data_count = p_max_rows;

         l_data_count := l_data_count + 1;
         clob_append (
            l_clob,
            l_cache,
            c_lf || '<tr><!--- row ' || to_char(l_data_count) || ' -->' || c_lf );

         if p_include_row_num then
            clob_append (
               l_clob,
               l_cache,
               '<td headers="row_num">' || to_char(l_data_count) || '</td>' || c_lf );
         end if;
         ...
         loop
            clob_append (
               l_clob,
               l_cache,
               '<td headers="' || lower(l_desc_tab(i).col_name) || '">' );

            if l_desc_tab(i).col_type = c_clob then
               sys.dbms_sql.column_value(l_cursor_id, i, l_buffer_clob);
               clob_append(
                  l_clob,
                  l_cache,
                  escape(substr(l_buffer_clob, 1, p_max_column_length)) ||
                     case
                        when length(l_buffer_clob) > p_max_column_length then
                        '...'
                     end );

            elsif l_desc_tab(i).col_type = c_xmltype then
               sys.dbms_sql.column_value(l_cursor_id, i, l_buffer_xmltype);

               if l_buffer_xmltype is not null then
                  ...
                  clob_append (
                     ...
                     escape(substr(l_buffer_clob, 1, p_max_column_length)) ||
                        case
                           when length(l_buffer_clob) > p_max_column_length
                           then '...'
                        end );
               end if;

            elsif l_desc_tab(i).col_type = c_long then
               sys.dbms_sql.column_value_long (
                  l_cursor_id,
                  i,
                  p_max_column_length,
                  0,
               ...
               clob_append (
                  l_clob,
                  l_cache,
                  escape(l_buffer_varchar2) ||
                     case
                        when l_buffer_long_length > p_max_column_length
                        then '...'
                     end );
               ...
               clob_append (
                  l_clob,
                  l_cache,
                  'Binary data type skipped - not supported for HTML' );

            else
               sys.dbms_sql.column_value(l_cursor_id, i, l_buffer_varchar2);
               clob_append(l_clob, l_cache, escape(l_buffer_varchar2));

            end if;

            clob_append(l_clob, l_cache, '</td>' || c_lf);
         end loop;

         clob_append (
            l_clob,
            l_cache,
            '</tr><!-- row ' || to_char(l_data_count) || ' -->' || c_lf );
      end loop;
   end create_data;
   --
begin
   l_cursor_id := sys.dbms_sql.to_cursor_number(l_data_cursor);
   describe_columns;

   if p_comment is not null then
      clob_append(l_clob, l_cache, escape(p_comment) || c_lflf);
   end if;

   clob_append(l_clob, l_cache, '<table>' || c_lf);
   create_header;
   create_data;
   clob_append(l_clob, l_cache, c_lf || '</table>' || c_lf);
   clob_flush_cache(l_clob, l_cache);
   close_cursor(l_cursor_id);
...
is
   ...
   l_value t_32kb;
begin
   l_key   := utl_escape_md_tab_text(p_key);
   l_value := utl_escape_md_tab_text(p_value);
   return
      '| ' ||
      case
         when nvl(length(l_key),   0) < 30
         then rpad(nvl(l_key  ,' '), 30, ' ')
         else l_key
      end ||
      ' | ' ||
      case
         when nvl(length(l_value), 0) < 43
         then rpad(nvl(l_value,' '), 43, ' ')
         else l_value
      end ||
      ' |'  || c_lf ||
      '| ------------------------------ ' ||
...
begin
   if p_value is null and not p_show_null_values then
      ...
      l_key   := utl_escape_md_tab_text(p_key);
      l_value := utl_escape_md_tab_text(substr(p_value, 1, p_value_max_length));
      return
         '| ' ||
         case
            when nvl(length(l_key),   0) < 30
...
begin
   ...
   -- create textbar: full block characters
      for i in 1..floor(p_value / l_value_one_character)
      loop
         l_return := l_return || unistr('\2588');
      end loop;
   ...
   -- are block character available in unicode for 1/8, 1/4, 3/8, 1/2, 5/8, 3/4, 7/8 and 1;
      case round((p_value / l_value_one_character - floor(p_value / l_value_one_character)) / 0.125)
         when 1 then         -- 1/8 = char U+258F
            l_return := l_return || unistr('\258F');
         when 2 then         -- 2/8 = char U+258E
            l_return := l_return || unistr('\258E');
         when 3 then         -- 3/8 = char U+258D
            l_return := l_return || unistr('\258D');
         when 4 then         -- 4/8 = char U+258C
            l_return := l_return || unistr('\258C');
         when 5 then         -- 5/8 = char U+258B
            l_return := l_return || unistr('\258B');
         when 6 then         -- 6/8 = char U+258A
            l_return := l_return || unistr('\258A');
         when 7 then         -- 7/8 = char U+2589
            l_return := l_return || unistr('\2589');
   ...
   -- fill up scale with shade
      if p_fill_scale = 1 then
         for i in 1..( p_width_block_characters - nvl(length(l_return), 0) )
         loop
            l_return := l_return || unistr('\2591');
         end loop;
      end if;
   end if;
...
function runtime_seconds (
   ...
   return number
is
   l_runtime interval day to second;
begin
   l_runtime := localtimestamp - p_start;

   return
      extract(hour   from l_runtime) * 3600 +
      extract(minute from l_runtime) *   60 +
      extract(second from l_runtime)        ;
end runtime_seconds;
...
begin
   if sys.utl_call_stack.dynamic_depth > 0 then
      ...
      loop
         l_subprogram := sys.utl_call_stack.concatenate_subprogram( sys.utl_call_stack.subprogram(i) );

         --exclude console package from the scope
         if instr ( upper(l_subprogram), 'CONSOLE.' ) = 0 then
            l_return := l_return ||
               case
                  when sys.utl_call_stack.owner(i) is not null
                  then sys.utl_call_stack.owner(i) || '.'
               end ||
               l_subprogram ||
               case
                  when sys.utl_call_stack.unit_line(i) is not null
                  then ', line ' || sys.utl_call_stack.unit_line(i)
               end;
         end if;
...
begin
   if g_saved_stack.count > 0 then
      l_return := l_return || '#### Saved Error Stack' || c_lflf;

      for i in 1 .. g_saved_stack.count
      loop
         l_return := l_return || '- ' || g_saved_stack (i) || c_lf;
      end loop;

      l_return := l_return || c_lf;
   end if;

   if sys.utl_call_stack.dynamic_depth > 1 then
      l_return := l_return || '#### Call Stack' || c_lflf;
   ...
   if sys.utl_call_stack.error_depth > 0 then
      l_return := l_return || '#### Error Stack' || c_lflf;

      for i in 1 .. sys.utl_call_stack.error_depth
      loop
         l_return := l_return || '- ORA-' ||
   ...
   if sys.utl_call_stack.backtrace_depth > 0 then
      l_return := l_return || '#### Error Backtrace' || c_lflf;
...
begin
   ...
   l_app_page_id := to_number(v(                   'APP_PAGE_ID' ));
   l_app_session := sys_context(  'APEX$SESSION' , 'APP_SESSION' );

   clob_append(l_clob, l_cache, '#### APEX Environment' || c_lflf);

   clob_append(l_clob, l_cache, 'APEX Session: ' || l_app_session || c_lflf);
   ...
   clob_append (
      ...
      '##### Application Items' ||
         case
            when l_app_id is not null
            then ' - APP_ID ' || to_char(l_app_id)
         end || c_lflf ||
   ...
   loop
      l_value := v(i.item_name);
      clob_append(l_clob, l_cache, to_md_tab_data(i.item_name, l_value));
   end loop;

   clob_append(l_clob, l_cache, c_lf);
   ...
   clob_append (
      ...
      '##### Page Items' ||
      case
         when g_conf_level < c_level_debug and l_app_page_id is not null
         then ' - APP_PAGE_ID ' || to_char(l_app_page_id)
      end || c_lflf ||
      to_md_tab_header('Item Name') );
   ...
   for i in (
      select item_name
       ...
       where application_id = l_app_id
         and page_id        = case
                                 when (select console.my_log_level from dual) >=
                                      (select console.level_debug from dual)
                                 then page_id
                                 else l_app_page_id
                              end )
   loop
...
is
   l_return t_32kb;
begin
   l_return := '#### CGI Environment' || c_lflf || to_md_tab_header;

   for i in 1 .. nvl(sys.owa.num_cgi_vars, 0)
   loop
      l_return := l_return ||
         to_md_tab_data (
...
is
   ...
   begin
      l_return := l_return || to_md_tab_data(p_key, p_value, p_show_null_values => true);
   end append_row;
   --
begin
   l_return := '#### Console Environment' || c_lflf || to_md_tab_header;
   ...
   if g_timers.count > 0 then
      l_return := l_return || '##### Running Timers' || c_lflf ||
         to_md_tab_header('Label', 'Start Time (localtimestamp)');
      l_index := g_timers.first;

      loop
         exit when l_index is null;
         append_row(l_index, to_char(g_timers(l_index), c_timestamp_format));
         l_index := g_timers.next(l_index);
      end loop;
   ...
   if g_counters.count > 0 then
      l_return := l_return || '##### Running Counters' || c_lflf ||
      to_md_tab_header('Label', 'Current Count');
      l_index := g_counters.first;

      loop
         exit when l_index is null;
         append_row(l_index, to_char(g_counters(l_index)));
         l_index := g_counters.next(l_index);
      end loop;
...
is
   ...
   is
   begin
      l_return := l_return || to_md_tab_data (
         p_key              => p_key,
   ...
   --
begin
   l_return := '#### User Environment' || c_lflf || to_md_tab_header;
   --
   append_row('ACTION');
...
function status
   return t_attribute_value_tab pipelined
is
   l_row t_attribute_value_row;
begin
   if g_conf_check_sysdate < sysdate then
      utl_set_session_conf;
   end if;

   l_row.attribute := 'c_version'               ; l_row.value :=                to_char( c_version                              ); pipe row(l_row);
   l_row.attribute := 'localtimestamp'          ; l_row.value :=                to_char( localtimestamp,          c_date_format ); pipe row(l_row);
   l_row.attribute := 'sysdate'                 ; l_row.value :=                to_char( sysdate,                 c_date_format ); pipe row(l_row);
   l_row.attribute := 'g_conf_check_sysdate'    ; l_row.value :=                to_char( g_conf_check_sysdate,    c_date_format ); pipe row(l_row);
   l_row.attribute := 'g_conf_exit_sysdate'     ; l_row.value :=                to_char( g_conf_exit_sysdate,     c_date_format ); pipe row(l_row);
   l_row.attribute := 'g_conf_client_identifier'; l_row.value :=                         g_conf_client_identifier                ; pipe row(l_row);
   l_row.attribute := 'g_conf_level'            ; l_row.value :=                to_char( g_conf_level                           ); pipe row(l_row);
   l_row.attribute := 'level_name(g_conf_level)'; l_row.value :=             level_name( g_conf_level                           ); pipe row(l_row);
   l_row.attribute := 'g_conf_check_interval'   ; l_row.value :=                to_char( g_conf_check_interval                  ); pipe row(l_row);
   l_row.attribute := 'g_conf_enable_ascii_art' ; l_row.value :=              to_string( g_conf_enable_ascii_art                ); pipe row(l_row);
   l_row.attribute := 'g_conf_call_stack'       ; l_row.value :=              to_string( g_conf_call_stack                      ); pipe row(l_row);
   l_row.attribute := 'g_conf_user_env'         ; l_row.value :=              to_string( g_conf_user_env                        ); pipe row(l_row);
   l_row.attribute := 'g_conf_apex_env'         ; l_row.value :=              to_string( g_conf_apex_env                        ); pipe row(l_row);
   l_row.attribute := 'g_conf_cgi_env'          ; l_row.value :=              to_string( g_conf_cgi_env                         ); pipe row(l_row);
   l_row.attribute := 'g_conf_console_env'      ; l_row.value :=              to_string( g_conf_console_env                     ); pipe row(l_row);
   l_row.attribute := 'g_counters.count'        ; l_row.value :=                to_char( g_counters.count                       ); pipe row(l_row);
   l_row.attribute := 'g_timers.count'          ; l_row.value :=                to_char( g_timers.count                         ); pipe row(l_row);
   l_row.attribute := 'g_saved_stack.count'     ; l_row.value :=                to_char( g_saved_stack.count                    ); pipe row(l_row);
   l_row.attribute := 'g_prev_error_msg'        ; l_row.value := utl_replace_linebreaks( g_prev_error_msg                       ); pipe row(l_row);
end status;
...
begin
   l_conf := utl_get_conf;
   l_row.attribute := 'conf_sysdate'    ; l_row.value := to_char( l_conf.conf_sysdate, c_date_format ); pipe row(l_row);
   l_row.attribute := 'conf_user'       ; l_row.value :=          l_conf.conf_user                    ; pipe row(l_row);
   l_row.attribute := 'level_id'        ; l_row.value := to_char( l_conf.level_id                    ); pipe row(l_row);
   l_row.attribute := 'level_name'      ; l_row.value :=          l_conf.level_name                   ; pipe row(l_row);
   l_row.attribute := 'check_interval'  ; l_row.value := to_char( l_conf.check_interval              ); pipe row(l_row);
   l_row.attribute := 'enable_ascii_art'; l_row.value :=          l_conf.enable_ascii_art             ; pipe row(l_row);
   l_row.attribute := 'client_prefs'    ; l_row.value :=          l_conf.client_prefs                 ; pipe row(l_row);
end conf;
...
function client_prefs
   return t_client_prefs_tab pipelined
is
   l_list  t_client_prefs_tab_i;
begin
   l_list := utl_get_client_prefs_tab;
...
begin
   ...
   if l_error_depth > 0 and l_backtrace_depth > 0 then
      if l_error_number != 6512 and l_error_msg != coalesce(g_prev_error_msg, 'null') then
         ...
         l_return := ' (line ' || to_char(l_backtrace_line) ||
...
exception
   when no_data_found then
      -- set defaults
      l_row.conf_id          := c_conf_id;
      l_row.conf_sysdate     := sysdate;
      l_row.conf_user        := 'autodefault';
      l_row.level_id         := c_level_error;
      l_row.level_name       := level_name(c_level_error);
      l_row.check_interval   := c_check_interval_default;
      l_row.enable_ascii_art := to_string(c_enable_ascii_art);
      return l_row;
end utl_get_conf;
...
function utl_get_client_prefs (
   ...
   return t_client_prefs_row
is
   l_all_prefs_csv   t_32kb := p_all_prefs_csv;
   l_csv             t_32kb;
   l_prefs           t_client_prefs_row;
   l_boolean_options t_int;
   l_start           t_int;
   l_stop            t_int;
begin
   if l_all_prefs_csv is not null then
      l_all_prefs_csv := replace(l_all_prefs_csv, c_cr, c_lf);
      l_start         := instr(l_all_prefs_csv, p_client_identifier || ',');
      l_stop          := instr(l_all_prefs_csv, c_lf, l_start);
      l_csv           := substr(l_all_prefs_csv, l_start, l_stop - l_start);
      ...
      if l_csv is not null then
         l_prefs.exit_sysdate := utl_csv_get_exit_sysdate(l_csv);
         ...
         if l_prefs.exit_sysdate >= sysdate then
            l_boolean_options      := utl_csv_get_boolean_options ( l_csv );
            l_prefs.level_id       := utl_csv_get_level           ( l_csv );
            l_prefs.check_interval := utl_csv_get_check_interval  ( l_csv );
            l_prefs.call_stack     := to_string ( bitand ( l_boolean_options, c_call_stack  ) = c_call_stack  );
            l_prefs.user_env       := to_string ( bitand ( l_boolean_options, c_user_env    ) = c_user_env    );
            l_prefs.apex_env       := to_string ( bitand ( l_boolean_options, c_apex_env    ) = c_apex_env    );
            l_prefs.cgi_env        := to_string ( bitand ( l_boolean_options, c_cgi_env     ) = c_cgi_env     );
            l_prefs.console_env    := to_string ( bitand ( l_boolean_options, c_console_env ) = c_console_env );
            l_prefs.level_name     := level_name ( l_prefs.level_id );
         end if;
      end if;
   end if;
...
begin
   l_conf  := utl_get_conf;
   l_prefs := replace(replace(replace(replace(l_conf.client_prefs,
      c_crlf , c_lf ),
      c_cr   , c_lf ),
      c_lflf , c_lf ),
      c_lflf , c_lf );
   l_len := length(l_prefs);

   loop
      l_lf := nvl(instr(l_prefs, c_lf, l_pos), 0);
      exit when l_lf = 0;

      if l_lf > l_pos then
         l_tab(l_tab.count + 1) := utl_csv_to_client_prefs (
            substr(l_prefs, l_pos, l_lf - l_pos) );
      end if;
...
is
   ...
   l_tab   t_client_prefs_tab_i;
begin
   l_tab := utl_get_client_prefs_tab;

   for i in 1 .. l_tab.count
   loop
      --filter out invalid client pefs
      if     l_tab(i).client_identifier is not null
         and l_tab(i).level_id          is not null
         and l_tab(i).call_stack        is not null
         and l_tab(i).user_env          is not null
         and l_tab(i).cgi_env           is not null
         and l_tab(i).user_env          is not null
         and l_tab(i).console_env       is not null
         and l_tab(i).check_interval    is not null
         and l_tab(i).exit_sysdate      is not null
         and l_tab(i).exit_sysdate      >= sysdate
         and l_tab(i).client_identifier != nvl(p_client_identifier_to_remove, 'NULL_VALUE_DETECTED')
      then
         l_prefs := l_prefs || utl_client_prefs_to_csv(l_tab(i));
      end if;
   end loop;

   if p_client_prefs_to_append.client_identifier is not null then
      l_prefs := l_prefs || utl_client_prefs_to_csv(p_client_prefs_to_append);
   end if;
...
function utl_remove_stale_client_prefs (
   ...
   return varchar2
is
   l_return t_32kb := p_all_prefs_csv;
begin
   return l_return; -- fixme: are we sure this works? Where is the code?
...
begin
   ...
   l_stop := instr(p_csv, ',', -1, 4) - 1;

   return substrb(substr(p_csv, 1, l_stop), 1, 64);
end;
...
begin
   --csv format: client_identifier,level,boolean_options,check_interval,exit_sysdate
   l_start  := instr(p_csv, ',', -1, 2) + 1;
   l_stop   := instr(p_csv, ',', -1, 1);
   l_return :=
      to_number (
         substr (
            p_csv,
   ...
   return
      case
         when l_return not between c_check_interval_min and c_check_interval_max
...
begin
   --csv format: client_identifier,level,boolean_options,check_interval,exit_sysdate
   l_start := instr(p_csv, ',', -1, 3) + 1;
   l_stop  := instr(p_csv, ',', -1, 2);
...
begin
   --csv format: client_identifier,level,boolean_options,check_interval,exit_sysdate
   l_start := instr(p_csv, ',', -1, 4) + 1;
   l_stop  := instr(p_csv, ',', -1, 3);
   l_return :=
      to_number (
         substr (
            p_csv,
   ...
   return
      case
         when l_return not between c_level_error and c_level_trace
         then c_level_error
         else l_return
      end;
...
function utl_csv_to_client_prefs (
   ...
   return t_client_prefs_row
is
   l_return          t_client_prefs_row;
   l_boolean_options t_int;
begin
   --csv format: client_identifier,level,boolean_options,check_interval,exit_sysdate
   l_boolean_options          := utl_csv_get_boolean_options   ( p_csv );
   l_return.exit_sysdate      := utl_csv_get_exit_sysdate      ( p_csv );
   l_return.client_identifier := utl_csv_get_client_identifier ( p_csv );
   l_return.level_id          := utl_csv_get_level             ( p_csv );
   l_return.check_interval    := utl_csv_get_check_interval    ( p_csv );
   l_return.call_stack        := to_string ( bitand ( l_boolean_options, c_call_stack  ) = c_call_stack  );
   l_return.user_env          := to_string ( bitand ( l_boolean_options, c_user_env    ) = c_user_env    );
   l_return.apex_env          := to_string ( bitand ( l_boolean_options, c_apex_env    ) = c_apex_env    );
   l_return.cgi_env           := to_string ( bitand ( l_boolean_options, c_cgi_env     ) = c_cgi_env     );
   l_return.console_env       := to_string ( bitand ( l_boolean_options, c_console_env ) = c_console_env );
   l_return.level_name        := level_name ( l_return.level_id );
...
begin
   ...
   if g_conf_client_identifier is null or g_conf_client_identifier = ':' then
      g_conf_client_identifier := c_client_id_prefix ||
...
begin
   l_conf  := utl_get_conf;
   l_prefs := utl_get_client_prefs(l_conf.client_prefs, g_conf_client_identifier);

   --If we have no real exit sysdate, we set it to 24 hours.
   --Session conf will be re-evaluated at least every 10 seconds.
   g_conf_exit_sysdate     := coalesce ( l_prefs.exit_sysdate         , sysdate + 1           );
   g_conf_check_interval   := coalesce ( l_prefs.check_interval       , l_conf.check_interval );
   g_conf_level            := coalesce ( l_prefs.level_id             , l_conf.level_id       );
   --
   g_conf_call_stack       := coalesce ( to_bool(l_prefs.call_stack)  , false                 );
   g_conf_user_env         := coalesce ( to_bool(l_prefs.user_env)    , false                 );
   g_conf_apex_env         := coalesce ( to_bool(l_prefs.apex_env)    , false                 );
   g_conf_cgi_env          := coalesce ( to_bool(l_prefs.cgi_env)     , false                 );
   g_conf_console_env      := coalesce ( to_bool(l_prefs.console_env) , false                 );
   --
   g_conf_enable_ascii_art := to_bool(l_conf.enable_ascii_art);
   --
...
begin
   ...
   if g_params.count > 0 then
      ...
      clob_append(l_row.message, l_cache, c_lf);
      g_params.delete;
   end if;
   ...
   if p_user_call_stack is not null then
      l_row.call_stack := substrb(p_user_call_stack, 1, 4000);
      ...
      l_row.call_stack := substrb(call_stack, 1, 4000);
   ...
   if p_apex_env or g_conf_apex_env then
      clob_append(l_row.message, l_cache, apex_env);
   end if;

   if p_cgi_env or g_conf_cgi_env then
      clob_append(l_row.message, l_cache, cgi_env);
   end if;

   if p_console_env or g_conf_console_env then
      clob_append(l_row.message, l_cache, console_env);
   end if;

   if p_user_env or g_conf_user_env then
      clob_append(l_row.message, l_cache, user_env);
   end if;

   clob_flush_cache(l_row.message, l_cache);

   l_row.log_time          := localtimestamp;
   l_row.level_id          := p_level;
   l_row.level_name        := level_name(p_level);
   l_row.permanent         := to_yn(p_permanent);
   l_row.session_user      := substrb ( sys_context ( 'USERENV', 'SESSION_USER'      ), 1, 32 );
   l_row.module            := substrb ( sys_context ( 'USERENV', 'MODULE'            ), 1, 48 );
   l_row.action            := substrb ( sys_context ( 'USERENV', 'ACTION'            ), 1, 32 );
   l_row.client_info       := substrb ( sys_context ( 'USERENV', 'CLIENT_INFO'       ), 1, 64 );
   l_row.client_identifier := substrb ( sys_context ( 'USERENV', 'CLIENT_IDENTIFIER' ), 1, 64 );
   l_row.ip_address        := substrb ( sys_context ( 'USERENV', 'IP_ADDRESS'        ), 1, 48 );
   l_row.host              := substrb ( sys_context ( 'USERENV', 'HOST'              ), 1, 64 );
   l_row.os_user           := substrb ( sys_context ( 'USERENV', 'OS_USER'           ), 1, 64 );
   l_row.os_user_agent     := substrb ( p_user_agent, 1, 200 );

   insert into console_logs values l_row returning log_id into l_row.log_id;
   commit;
