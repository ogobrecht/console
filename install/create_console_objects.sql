...
REMARK: FOR DEVELOPMENT ONLY - UNCOMMENT THE NEXT TWO LINES TEMPORARELY WHEN YOU NEED IT
...
declare
  ...
  v_count pls_integer;
begin
  select count(*) into v_count from user_tables where table_name = v_name;
  if v_count = 0 then
    dbms_output.put_line('Table not found, run creation command.');
    ...
    dbms_output.put_line('Table found, no action required.');
  end if;
end;
/
