...
begin
   ...
   return sys.utl_lms.format_message (
      v_message, p0, p1, p2, p3, p4, p5, p6, p7, p8, p9 );
end format;
         ...
         v_return :=
            ...
            replace (
               replace (
                  ...
                  rpad(v_app_id, 9, ' ') ),
               '#LOG_ID##########',
               rpad(nvl(to_char(v_log_id), 'this.log_id'), 17, ' ') ) ||
            case p_type
               when 'html' then '</pre>'
               when 'md'   then '```'
            end;
   ...
   begin
      ...
      for i in 1..v_col_count loop
         clob_append (
            v_clob,
            v_cache,
            '<th id="' || lower(v_desc_tab(i).col_name) || '">' ||
               initcap(replace(v_desc_tab(i).col_name, '_', ' ')) ||
               '</th>' || c_lf );
      end loop;
   ...
   begin
      loop
            ...
            if v_desc_tab(i).col_type = c_clob then
               ...
               clob_append(
                  ...
                  escape(substr(v_buffer_clob, 1, p_max_column_length)) ||
                     case
                        when length(v_buffer_clob) > p_max_column_length then
                        '...'
                     end );
               ...
               if v_buffer_xmltype is not null then
                  ...
                  clob_append (
                     ...
                     escape(substr(v_buffer_clob, 1, p_max_column_length)) ||
                        case
                           when length(v_buffer_clob) > p_max_column_length
                           then '...'
                        end );
               end if;
               ...
               clob_append (
                  ...
                  escape(v_buffer_varchar2) ||
                     case
                        when v_buffer_long_length > p_max_column_length
                        then '...'
                     end );
         ...
         clob_append (
            v_clob,
            v_cache,
            '</tr><!-- row ' || to_char(v_data_count) || ' -->' || c_lf );
      end loop;
   end create_data;
   --
begin
...
begin
   ...
   return '| ' ||
      case
         when nvl(length(v_key),   0) < 30
         then rpad(nvl(v_key  ,' '), 30, ' ')
         else v_key
      end || ' | ' ||
      case
         when nvl(length(v_value), 0) < 43
         then rpad(nvl(v_value,' '), 43, ' ')
         else v_value
      end || ' |'  || c_lf ||
      '| ------------------------------ ' ||
      '| ------------------------------------------- |' || c_lf;
...
begin
   ...
   clob_append (
      ...
      '##### Page Items' ||
      case
         when g_conf_level < c_level_debug and v_app_page_id is not null
         then ' - APP_PAGE_ID ' || to_char(v_app_page_id)
      end || c_lflf ||
      to_md_tab_header('Item Name') );
...
begin
   ...
   if g_timers.count > 0 then
      v_return := v_return || '##### Running Timers' || c_lflf ||
   ...
   if g_counters.count > 0 then
      v_return := v_return || '##### Running Counters' || c_lflf ||
